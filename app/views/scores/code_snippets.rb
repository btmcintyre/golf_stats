...
<script>
    ...
    //var array = "<%= j @stableford_pts %>";
    var array = [1,2,3];

    jQuery.each( myJsArray, function( i, val ) {
  alert(val);
});

var myJsArray = <%=raw @tableford_pts%>;



$(alert(myJsArray[4]));

jQuery.each(myJsArray, function(index, item) {
    // do something with `item` (or `this` is also `item` if you like)
});

$('#'+'pts_'+<%=@hole_number%>).text('<%=@pts%>');


url: "<%= Rails.application.routes.url_helpers.score_calcs_score_path(:id => @score.id) %>",
